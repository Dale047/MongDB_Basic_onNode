...
/*
	Model.update(conditions, doc, [projection], [options], [callback])
	 - 更新所有符合条件的文档
	Model.updateMany(conditions, doc, [projection], [options], [callback])
	 - 更新所有符合条件的文档
	Model.updateOne(conditions, doc, [projection], [options], [callback])
	 - 更新所有符合条件的第一个文档
	 ...
	 callback 回调函数：结果集通过回调函数返回
		如果不传没有结果集
*/
stuModel.updateOne({name:"Dale"},{$set:{age:20}},function(err){
	if(!err){
		console.log("修改成功");
	}
});